...
const validPsk = (psk: unknown): boolean => {
  ...
  if (!/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(psk)) {
    return false;
  }
...
const validateSemantics = (record: CommunityMesh, filename: string): void => {
  ...
  for (const profile of profiles) {
    ...
    if (profileIds.has(id)) throw new Error(`duplicate profile id in ${filename}: ${id}`);
    profileIds.add(id);
    ...
    if (radio.overrideFrequencyMHz !== undefined && !licensed) {
      throw new Error(`override frequency requires a licensed profile in ${filename}`);
    }

    const channels = [profile.primaryChannel, ...((profile.optionalChannels as unknown[] | undefined) ?? [])] as Array<Record<string, unknown>>;
    for (const channel of channels) {
      if (Buffer.byteLength(channel.name as string, "utf8") > 12) {
        throw new Error(`channel name exceeds 12 bytes in ${filename}`);
      }
      if (!validPsk(channel.pskBase64)) throw new Error(`invalid PSK in ${filename}`);
      if (licensed && channel.pskBase64 !== null) {
        throw new Error(`licensed profile contains an encrypted channel in ${filename}`);
      }
    }
  }
};

export const loadCommunityMeshes = (directory = DATA_DIRECTORY): CommunityMesh[] =>
